...
const TECHNICAL_META_PATTERNS = [
  /(?:代码|编程|bug|报错|服务器|部署|github|仓库|浏览器|api|token|模型|prompt|向量|embedding|世界书|预设|文档|文本|数据|功能|界面)/i,
];
...
const SIGNAL_PATTERNS: ReadonlyArray<{
...
}> = [
  {
    ...
    patterns: [
      /(?:不舒服|难受|头疼|头痛|胃疼|胃痛|腰疼|腰痛|失眠|睡不好|发烧|受伤|撑不住|快崩溃|情绪崩溃)/i,
      /(?:^|[，,。！？!?\s])(?:我)?(?:今天|最近|刚刚|现在|这会儿)?(?:真的|确实)?(?:有点|有些|稍微|好|很|太)?(?:困(?:了|得|啊|呀|呢|吧|～|~|！|!|。|$)|累(?:了|坏|得|到|啊|呀|呢|吧|～|~|！|!|。|$))/i,
    ],
  },
  {
    signal: 'care_needed',
    patterns: [
      /(?:不舒服|难受|头疼|头痛|胃疼|胃痛|腰疼|腰痛|发烧|受伤|失眠|睡不好|没吃(?:饭|东西|药)|忘了吃(?:饭|东西|药)|累坏|撑不住|快崩溃|情绪崩溃)/i,
    ],
  },
  {
    signal: 'refusal',
  ...
  {
    signal: 'pause_or_redirect',
    patterns: [
      /(?:先不说了|暂时不说|不想说了|别问了|先别问|别说了|到这里吧|先这样吧|换个话题|聊点别的|说别的|跳过这个|不聊这个)/i,
