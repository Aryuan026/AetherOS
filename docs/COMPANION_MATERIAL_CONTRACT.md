...
- 对方只留下短回应时，角色可以带入一项自己的具体内容，让对话从角色这一侧继续生长。

这层只在 `refusal / no_advice_chat / care_needed / mild_discomfort /
pause_or_redirect / reentry / low_signal` 等现场信号出现时投影，品质包不会常驻。
Chat 另有一条不指定角色实现方式的轻量对话底色；真正命中短回应时，才补当前角色的
具体 realization。一次只选择一个
...
方向与来源组统计。短回应的共享方法来自 owner 已审读的对话方法，不冒充短信证据；
五位角色怎样带入侧枝仍由各自 realization 决定。自建角色没有内置 realization 时仍使用同一共享地板，但具体落法
...
连续的“嗯 / ？ / 短笑声”也遵守同一相邻去重规则；带有具体内容的笑声仍按普通分享处理。
