...
- 共享互动品质是否只出现一次，并且同一触发下五位角色仍有不同的正向出口；
- “尊重拒绝”是否被误写成统一顺从，“关心不接管”是否被误写成统一照护流程。
- 技术词、复合词和多人共创中的 NPC 台词是否被误判成玩家身体状态或边界；
- 连续两轮同一信号是否重复注入静态 system 区块；
- surface 格式合同是否强迫所有角色采用相同的多段、电影化或先承接后建议结构。
...
- 跨角色共同品质不是角色指纹，但也不是废料：共享基线只保留一份；历史证据若支持
  角色特有实现，则进入 boundary/care/repair style。运行时同轮只投影一个共享原则与
  一个角色实现，不占用普通素材的单条名额。
- 共享品质与角色实现应在同一段里提供“可选出口”，不能排成共同底色、角色落法两个
  固定步骤；已选中一条素材也不能仅凭 kind 就宣称当前品质已经被覆盖。
- 当前用户边界需要第一人称或直接指令置信度；技术元话题和多人共创文本默认不把
  句中角色的疼痛、拒绝或暂停归到玩家本人。
