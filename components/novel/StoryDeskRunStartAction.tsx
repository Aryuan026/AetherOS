...
export const StoryDeskRunStartAction: React.FC<StoryDeskRunStartActionProps> = ({
    ...
    const confirmStart = async () => {
        ...
        try {
            ...
            setError(cause instanceof Error ? cause.message : '线路启动失败，请重新打开剧情台后再试。');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4" data-testid={`story-run-start-review-${runId}`}>
            <h5 className="text-sm font-black text-slate-800">把这条线路设为进行中？</h5>
            <p className="mt-2 text-[11px] leading-relaxed text-slate-500">
                这一步只选择当前要推进的线路。不会生成场景、不会调用 AI，也不会写入经历、记忆或角色此刻状态。
            </p>
            {error && (
