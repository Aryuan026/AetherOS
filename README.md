...
| 👤 **外观** | 改系统外观，让它看起来像你的手机 |
| 📥 **旧日迁入** | 导入 TXT / Word 历史对话，校对后接到角色与面具关系下 |
...
- 已覆盖常见的 `user:` / `assistant:` 加 `timestamp:` 导出格式；无法可靠判断的说话人、时间和分段会留给人类逐条确认，不会强编成剧情或关系结论。
- 文件在当前设备的浏览器里解析并写入 IndexedDB，不上传历史原文，也不借用部署服务器处理。单文件上限 64 MiB、最多 100,000 个文本单元；特别大的文件更适合在电脑网页版导入。
- 全量校对工作区会分块落在本地，刷新后可以继续；正式归档再按日期切成稳定的小块，不要求把一天几千条消息同时塞进页面。
- `对话日历` 按当前角色 × 面具关系查看记录。打开某一天后使用全屏阅读器和分页窗口，人类可见的当天总条数不设业务上限。
- 对话可以剪藏为保留原文与来源的素材；剪藏不会自动改写人设、关系、记忆卡或提示词。
- 剪藏库顶部可以检索当前关系的全部本地聊天。结果显示日期、说话者与关键词高亮，点击会回到原始消息；当前是明确的关键词检索，语义/向量召回仍是后续升级位。
